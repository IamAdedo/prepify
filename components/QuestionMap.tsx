    ...
    <aside className="w-full lg:w-64 bg-white p-4 rounded border-2 border-gray-300 shadow-inner select-none flex flex-col">
      ...
      <div className="grid grid-cols-5 sm:grid-cols-8 lg:grid-cols-4 gap-2 max-h-64 lg:max-h-96 overflow-y-auto p-1">
        {Array.from({ length: totalQuestions }).map((_, idx) => {
          ...
          const isVisited = visitedQuestions.includes(idx + 1);
          ...
          else if (isVisited) btnStyle = "bg-gray-300 text-gray-800 border-gray-400";

          if (isCurrent) {
            btnStyle = "bg-jamb-blue text-white border-jamb-gold ring-2 ring-jamb-gold font-bold scale-105";
          }
      ...
      <div className="mt-4 pt-3 border-t text-[11px] space-y-1.5 text-gray-600">
        <div className="flex items-center space-x-2">
          <span className="w-3 h-3 bg-jamb-blue rounded-full border"></span>
          <span>Current</span>
        </div>
        <div className="flex items-center space-x-2">
